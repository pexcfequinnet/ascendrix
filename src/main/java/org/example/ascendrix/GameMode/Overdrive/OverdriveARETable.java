...
public class OverdriveARETable {
    ...
    public static long getLineClearDelay(int level) {
        if (level == 1500) return frame_to_ns(4);
    ...
    public static long getClearAnimDelay(int level) {
        if (level >= 1500) return frame_to_ns(6);
        if (level >= 500)  return frame_to_ns(3);
        if (level >= 200)  return frame_to_ns(4);
        if (level >= 100)  return frame_to_ns(5);
        return frame_to_ns(6);
    }
